...
export const actions: Actions = {
	default: async ({ request, cookies }) => {
		...
		if (!nombre || !apellido_pat || !telefono || !correo || !password || !nombre_negocio || !giro || !ramo) {
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?)`
		).run(usuario.lastInsertRowid, nombre_negocio, tiene_razon_social, razon_social, rfc, giro, ramo);
